...
func Down(db *pgxpool.Pool, dir string, opts ...OptionsFunc) error {
	option := &options{}
	for _, f := range opts {
		f(option)
	}
...
func DownTo(db *pgxpool.Pool, dir string, version int64, opts ...OptionsFunc) error {
	option := &options{}
	for _, f := range opts {
		f(option)
	}
...
// target version.
func downToNoVersioning(db *pgxpool.Pool, migrations Migrations, version int64) error {
