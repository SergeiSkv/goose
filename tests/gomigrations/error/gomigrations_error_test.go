...
import (
	...
	_ "github.com/SergeiSkv/goose/v3/tests/gomigrations/error/testdata"
)
...
func TestGoMigrationByOne(t *testing.T) {
	...
	// Registered Go migration run outside a goose tx using*pgx.Conn.
	...
	var count int
	err = db.QueryRow(context.Background(), "SELECT COUNT(*) FROM foo").Scan(&count)
	check.NoError(t, err)
